...
import { chapterLearningManager } from './chapter-learning.js';
...
export class LearningEngine {
  ...
  async processInteraction({ studentId = 'default-student', message, mode = 'chat', courseId = '', attachedDocId = '', userApiKey = '', image = null }) {
    ...
    // 1. Autonomous Agent Routing Check: Plan de A à E dirigé par le Tuteur
    const activeSession = db.getActiveSessionForStudent(studentId);
    const chapterMatch = chapterLearningManager.findChapterForQuery(message, courseId);
    
    // If student is in 'apprendre' mode, or has an active autonomous session waiting for response, or asks to learn a chapter
    if (modeNormalized === 'apprendre' || (activeSession && activeSession.pendingExercise) || (chapterMatch && (lower.includes('apprendre') || lower.includes('plan') || lower.includes('cours') || lower.includes('étudier') || lower.includes('comprendre')))) {
      if (activeSession) {
        const stepResult = await chapterLearningManager.processStep({
        ...
        return {
          answer: stepResult.tutorAnswer,
          mode: 'apprendre',
          learningAction: 'autonomous_plan',
          learningPlan: stepResult.learningPlan,
          interactiveExercise: stepResult.interactiveExercise,
          recommendedVideo: stepResult.recommendedVideo,
          quickActions: stepResult.quickActions,
          sources: stepResult.sources || ragResult.sources,
          sessionId: activeSession.id
        };
      } else if (chapterMatch) {
        const initResult = await chapterLearningManager.initSession({
          studentId,
          courseId: chapterMatch.courseId,
          chapterId: chapterMatch.chapterId,
          userApiKey
        });
        return {
          answer: initResult.tutorAnswer,
          mode: 'apprendre',
          learningAction: 'autonomous_plan_init',
          learningPlan: initResult.learningPlan,
          interactiveExercise: initResult.interactiveExercise,
          recommendedVideo: initResult.recommendedVideo,
          quickActions: initResult.quickActions,
          sources: initResult.sources || ragResult.sources,
...
Retourne UNIQUEMENT un objet JSON :
{
  ...
  // Chapter Learning Engine Integration (Mode Apprendre par Chapitre)
  getLearningCourses() {
    return chapterLearningManager.getAvailableCourses();
  }

  initChapterLearningSession(params) {
    return chapterLearningManager.initSession(params);
  }

  processChapterLearningStep(params) {
    return chapterLearningManager.processStep(params);
  }

  getChapterWhereAreWe(sessionId) {
    return chapterLearningManager.getWhereAreWe(sessionId);
  }

  getChapterAssessment(sessionId) {
    return chapterLearningManager.getChapterAssessment(sessionId);
  }

  submitChapterAssessment(sessionId, studentAnswers) {
    return chapterLearningManager.submitChapterAssessment(sessionId, studentAnswers);
  }
}

export const learningEngine = new LearningEngine();
export { chapterLearningManager };
