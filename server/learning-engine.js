...
export class LearningEngine {
  ...
  async processInteraction({ studentId = 'default-student', message, mode = 'chat', courseId = '', userApiKey = '' }) {
    const profile = db.getStudentProfile(studentId);
...
RÉPONSE DU TUTEUR ACADÉMIQUE :`;
    ...
    if (result.success) {
      ...
      // High-quality deterministic fallback response if no API key is set yet
      confidence = 0.85;
      modelUsed = 'Secours Pédagogique (Local)';
      if (modeNormalized === 'apprendre') {
        answerText = `Voici une explication structurée pour progresser sur ce concept :\n\n` +
        ...
        answerText = `RÉVISION EXAMEN (Basée sur les annales de la faculté) :\n\n` +
        ...
        answerText = `EXERCICE D'ENTRAÎNEMENT GUIDÉ :\n\n` +
        ...
        answerText = `Je suis à ton écoute pour t'aider dans tes études. Tu peux me poser des questions sur les cours d'Algorithmique, d'Analyse Mathématique, de Mécanique ou de Bases de Données, consulter les examens corrigés, ou basculer sur les modes **Apprendre**, **Révision** ou **S'exercer** !`;
      }
    }
